...
event.on('static-serve', function (req, res, context = '') {
  ...
  const url =
    path.extname(req.url).indexOf('.') === -1
      ? `/${ENV.APP_WWW_INDEX}`
      : req.url.toLowerCase().replace(`/${context}`.toLowerCase(), '');
  console.log(`SERVE :: ${req.url} > ${url}`);
    ...
    fs.mkdir(`${path.join(__dirname, ENV.APP_MOCK_FOLDER)}${location}`, { recursive: true }, (err) => {
      if (err) {
        ...
        try {
          ...
          const stream = fs.createWriteStream(`${path.join(__dirname, ENV.APP_MOCK_FOLDER)}${url}.json`, {
            flags: 'w',
          });
...
//   const param = ((req.params || {}).email || '').split('@');
//   const email = param.length > 1 ? param[1] : '';
//   let url = (req.url || '').split('/');
//   if (email.indexOf('ey.com') !== -1) {
//     url.pop();
//     url = url.join('/');
//     req.url = `${url}/nexus.demo@ey.com`;
//   }
//   console.log(`${req.url}`);
...
app.get('/*', function (req, res) {
  console.log(`SPA Redirect :: ${req.url}`);
  event.emit('request-index', req, res);
});

// Startup the Server
const port = ((process || {}).env || {}).PORT ? process.env.PORT : ENV.APP_PORT;
const server =
...
const url = `${ENV.APP_PROTOCOL}://${ENV.APP_HOST}${port ? ':' + port : ''}`;
console.log(`**** VELOCITY server started @ ${new Date()} ****`);
console.table(server.address());
console.log(`Check out the SPA Web Server    @ ${url}`);
console.log(`Check out the Proxy/Mock Server @ ${url}${ENV.APP_API_ROUTE}/`);
