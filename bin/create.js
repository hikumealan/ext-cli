...
const main = (req, next = init) => {
  ...
  switch (next) {
    case 'get-project-framework':
      ...
      prompt(
        req.input.framework,
        `Please provide the framework you want to use. \nOptions: ${(req.cmds[req.input.cmd] || []).join(', ')} \n`,
        (input) => {
          input = (input || '').toLowerCase();
          if (input.length && (req.cmds[req.input.cmd] || []).includes(input)) {
...
const projectComplete = (req) => {
  ...
  try {
    ...
    const script = (scripts[`npx:template:get`] || '')
      .replace(/\$npm_config_cli/g, req.env.repo)
      .replace(/\$npm_config_template/g, template);
    const results = execSync(`${script}`).toString();
    const { data } = JSON.parse(results);
