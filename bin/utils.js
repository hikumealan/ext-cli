...
const download = (url, des, cb) => {
  ...
  try {
    req
      .get(src.href, function (response) {
        response.pipe(file);
        file.on('finish', function () {
          file.close(res);
        });
...
; end auth token`
    .replace(/\$npm_config_username/g, user)
    .replace(/\$npm_config_id/g, id)
    .replace(/\$npm_config_password/g, token)
    .replace(/\$$npm_config_email/g, email);
};
